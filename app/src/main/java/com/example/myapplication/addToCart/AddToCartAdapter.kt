...
import com.example.myapplication.databinding.AddtocartscreenlistBinding
...
class AddToCartAdapter (private val onClickListener: OnClick ) :  ListAdapter<GetCartList, RecyclerView.ViewHolder>(completeListDiffCallback()) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == DATA_VIEW_TYPE)
            (holder as AddToCartAdapter.ViewHolder).bind(getItem(position)!!,onClickListener,position)
        else (holder as AddToCartAdapter.LoadingHolder).bind(state)
        //     holder.bind(getItem(position)!!, onClickListener,position)
    }

    override fun onCreateViewHolder(parent: ViewGroup,
                                    viewType: Int): RecyclerView.ViewHolder {
        return if(viewType == DATA_VIEW_TYPE) AddToCartAdapter.ViewHolder.from(parent) else AddToCartAdapter.LoadingHolder.from(parent)
        // return  ViewHolder.from(parent)
    }

    class ViewHolder private constructor(val binding: AddtocartscreenlistBinding) : RecyclerView.ViewHolder(binding.root){
        ...
        companion object {
            fun from(parent: ViewGroup): ViewHolder {
                ...
                val binding = AddtocartscreenlistBinding.inflate(layoutInflater, parent, false)
...
class OnClick(val clickListener: (marsProperty: GetCartList, type:Int, index:Int) -> Unit) {
    ...
    fun onDelete(marsProperty: GetCartList, index:Int) = clickListener(marsProperty,3,index)
    fun add(marsProperty: GetCartList, index:Int) = clickListener(marsProperty,4,index)
    fun subtract(marsProperty: GetCartList, index:Int) = clickListener(marsProperty,5,index)
}
